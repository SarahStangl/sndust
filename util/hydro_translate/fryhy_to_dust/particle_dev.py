...
def load_particle_ezra_dev( h5fn: str, mdl_idx: int, p_idx: int) -> Particle:
    p = Particle()
    hf = h5.File(h5fn, 'r')

    pd = hf['particles'][str(p_idx)]

    # composition: element names
    cmpk = list(pd['composition'].keys())

    p.pid = p_idx
    # p.sid = 

    p.times = pd['times'][()]
    p.temperatures = pd['temperatures'][()]
    p.densities = pd['densities'][()]
    p.mass = pd['mass'][()]
    p.position = pd['position'][()]
    p.volume = pd['volume'][()]
    p.composition = dict({key: pd['composition'][key] for key in cmpk})
