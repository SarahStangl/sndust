...
def duster(settings, model_id, zone_id):
    ...
    if restart:
        fName = './output_M'+str(model_id).zfill(3)+'/old'+str(zone_id).zfill(4)+'.hdf5' #change the file name to match your naming scheme
        resF = h.File(fName,'r')
        keys = list(resF['root'].keys())
        data = resF['root'][keys[-1]][-1]
        t_start = data[1] #time of the last chunk
        NG = net._NG + 7 #hard coded, need to change this if you change what data is saved to the hdf5 file
        conc = list(data)[7:NG]
